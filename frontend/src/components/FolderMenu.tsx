import { faFolder, faFolderOpen, faPlus, faXmark } from "@fortawesome/free-solid-svg-icons";
...
const FolderMenu: React.FC<FolderMenuProps> = ({
  ...
  return (
    <>
      ...
      >
        <FontAwesomeIcon icon={faFolderOpen} onClick={() => setIsOpen(!isOpen)} />
      </button>
        ...
        >
          ...
          >
            <div className="w-full flex justify-end absolute -top-7 right-0">
              <button onClick={() => setIsOpen(!isOpen)} className="text-xl hover:text-amber-300">
            ...
            <div className="border border-neutral-500 w-full rounded p-3 pt-2">
              <ul className="text-sm">
                ...
                {selectedFeeds.length < 1 && <p className="text-neutral-300">no feeds selected</p>}
              </ul>
            </div>
                ...
                {folderNames
                  ...
                  .map((name, index) => (
                    <button
                      ...
                      className="text-white hover:text-amber-300 transition-color duration-150"
                      key={index}
                    >
              ...
              {!creatingNewFolder ? (
                ...
                >
                  ...
                  <div className="grid grid-flow-col justify-stretch gap-2 w-full">
                    <button
                      type="button"
                      className="hover:text-neutral-500 text-white transition-color duration-150 ease-in-out bg-neutral-900 rounded"
                      onClick={() => {
                        setCreatingNewFolder(false);
                        setNewFolderName("");
                      }}
                    ...
                    <button
                      type="submit"
                      className="hover:text-amber-300 text-white transition-color duration-150 ease-in-out bg-neutral-900 rounded"
                    >
                      submit
                    </button>
                  </div>
